...
const SlideItem = ({slide, slideLabel, className}:{slide: Slide, slideLabel?: string, className?: string}) => {
  return (
    <div>
      ...
      <div className={className || "bg-black aspect-[16/9] flex items-center justify-center p-4"} style={{fontSize: 'clamp(0.75rem, 1.5vw, 1.75rem)'}}>
        <div className="space-y-1 text-center">
          {slide.map((line: ParsedLine, lineIndex: number) => {
            ...
            if (line.isHeading) {
              return <div key={lineIndex} className="font-bold" style={{fontSize: 'clamp(1.3rem, 2.625vw, 3rem)'}}>{line.text}</div>;
            }
            if (line.isEmpty) {
