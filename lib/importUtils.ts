...
import { createSong, createVersionWithLineage, findVersionBySongTitleAndLabel } from './songsRepository';
import sql from './db';
...
export const importFromDirectories = async (
  ...
  dryRun: boolean,
  onResult?: (type: 'song' | 'speech', result: ImportResult) => void
): Promise<{ songResults: ImportResult[]; speechResults: ImportResult[] }> => {
  ...
  return { songResults, speechResults };
};
