...
const AdminFeedbackPage = () => {
  ...
  const renderSingability = (votes: Vote[]) => {
    if (votes.length === 0) return null;
    const vote = votes[0];
    const color = vote.weight > 0 ? 'text-green-400' : vote.weight < 0 ? 'text-red-400' : 'text-gray-400';
    return <span className={`text-xs ${color}`} title={vote.type}>{vote.weight}</span>;
  };
  ...
  return (
    <div className="p-4">
      ...
      <p className="text-gray-400 text-sm mb-4">{users.length} users have provided feedback</p>
        ...
        <table className="text-sm border-collapse">
          <thead>
            <tr className="border-b border-gray-700">
              ...
              {users.map(user => {
                const key = user.userId || 'anonymous';
                return (
                  <th key={key} colSpan={3} className="text-center py-2 px-1 text-white font-medium border-l border-gray-700 min-w-[120px]">
              ...
              {users.map(user => {
                ...
                return (
                  <React.Fragment key={key}>
                    <th className="py-1 px-1 border-l border-gray-700">Qual</th>
                    <th className="py-1 px-1">Sing</th>
          ...
          <tbody>
            {orderedVersionIds.map(versionId => {
              ...
              return (
                <tr key={versionId} className="border-b border-gray-800 hover:bg-gray-900">
                  <td className="py-1 px-2 text-gray-300 sticky left-0 bg-gray-950 z-10 font-georgia">{version.song_title}</td>
                  {users.map(user => {
                    const key = user.userId || 'anonymous';
                    const feedback = userFeedbackByVersion[versionId]?.[key] || { qualityVotes: [], singabilityVotes: [], comments: [] };
                    return (
                      <React.Fragment key={key}>
                        <td className="py-1 px-1 border-l border-gray-800">{renderQualityDots(feedback.qualityVotes)}</td>
                        <td className="py-1 px-1 text-center">{renderSingability(feedback.singabilityVotes)}</td>
                        <td className="py-1 px-1">{renderCommentCell(feedback.comments)}</td>
                      </React.Fragment>
                    );
                  })}
                </tr>
